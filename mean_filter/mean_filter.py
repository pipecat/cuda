...
def compile_shift_extension():
    cuda_source = Path("mean_filter_kernel.cu").read_text()
    cpp_source = "torch::Tensor shift_filter(torch::Tensor image);"

    extension = load_inline(
        name="shift_filter_extension",
        cpp_sources=cpp_source,
        cuda_sources=cuda_source,
        functions=["shift_filter"],
        with_cuda=True,
        extra_cuda_cflags=["-O2"],
        # build_directory="./cuda_build"
    )
...
def main():
    ...
    ext = compile_shift_extension()
    y = ext.shift_filter(x)
    write_png(y.cpu(), "output_shift.png")
